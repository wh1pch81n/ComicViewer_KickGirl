...
//      NIDWARNING(@"Unable to initialize reachability reference");
			self = nil;
		} else {
...
//        NIDINFO(@"Reachability observer initialized for Local Wifi");
      } else if (address->sa_len == sizeof(struct sockaddr_in) && address->sa_family == AF_INET) {
//        NIDINFO(@"Reachability observer initialized with IP address: %@.", _host);
      }
...
//    NIDINFO(@"Reachability observer initialized with hostname %@", hostNameOrIPAddress);
    if (_reachabilityRef == NULL) {
//      NIDWARNING(@"Unable to initialize reachability reference");
			self = nil;
		} else {
...
- (RKReachabilityNetworkStatus)networkStatus {
    ...
    //        NIDPRINT(@"Reachability observer %@ has not yet established reachability. networkStatus = %@", self, @"RKReachabilityIndeterminate");
		return RKReachabilityIndeterminate;
	}
	
  //    NIDPRINT(@"Reachability Flags: %@\n", [self reachabilityFlagsDescription]);
  ...
  //    NIDPRINT(@"Reachability observer %@ determined networkStatus = %@", self, [self stringFromNetworkStatus:status]);
	return status;
}
...
- (void)validateIntrospection {
  //MSZ Turned off as this is just noise
...
- (BOOL)isNetworkReachable {
  ...
  //    NIDPRINT(@"Reachability observer %@ determined isNetworkReachable = %d", self, reachable);
	return reachable;
}
...
- (BOOL)isConnectionRequired {
  ...
  //    NIDPRINT(@"Reachability observer %@ determined isConnectionRequired = %d", self, required);
	return required;
}
...
- (void)scheduleObserver {
  ...
  //    NIDPRINT(@"Scheduling reachability observer %@ in main dispatch queue", self);
	if (! SCNetworkReachabilitySetCallback(_reachabilityRef, ReachabilityCallback, &context)) {
//    NIDWARNING(@"%@: SCNetworkReachabilitySetCallback() failed: %s", self, SCErrorString(SCError()));
    return;
  }
  
  if (! SCNetworkReachabilitySetDispatchQueue(_reachabilityRef, dispatch_get_main_queue())) {
//    NIDWARNING("%@: SCNetworkReachabilitySetDispatchQueue() failed: %s", self, SCErrorString(SCError()));
    return;
  }
}
...
- (void)unscheduleObserver {
	if (_reachabilityRef) {
    //        NIDPRINT(@"%@: Unscheduling reachability observer from main dispatch queue", self);
    if (! SCNetworkReachabilitySetDispatchQueue(_reachabilityRef, NULL)) {
//			NIDWARNING("%@: SCNetworkReachabilitySetDispatchQueue() failed: %s\n", self, SCErrorString(SCError()));
			return;
		}
	} else {
    //        NIDPRINT(@"%@: Failed to unschedule reachability observer %@: reachability reference is nil.", self, _reachabilityRef);
  }
}
...
//    NIDINFO(@"Network availability has been determined for reachability observer %@", self);
